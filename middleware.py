...
def _downstream_enabled(key: str) -> bool:
    data = config.raw()
    ds = data.get("downstream", {})
    val = ds.get(key, True) if isinstance(ds, dict) else True
    if isinstance(val, bool):
        return val
    if isinstance(val, str):
        return val.strip().lower() in {"1", "true", "yes", "on"}
...
def verify_downstream_enabled(key: str):
    """Return a FastAPI dependency that checks whether a downstream endpoint is enabled.

    Usage::

        @router.post("/chat/completions", dependencies=[Depends(verify_downstream_enabled("chat_completions"))])

    When the switch is off the dependency raises 404 so the route behaves as
    though it does not exist.
    """

    async def _guard() -> None:
        if not _downstream_enabled(key):
