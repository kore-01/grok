...
class AccountRuntimeTable:
    ...
    # --- Per-account used-lockout (uint32 epoch-seconds; 0 = no lockout) ---
    # Set to now_s + 15 after each successful use; selector skips locked accounts
    used_lockout_until_s_by_idx: "array.array[int]" = field(
        default_factory=lambda: array.array("L")
    )
    ...
    def _append_slot(
        ...
        self.used_lockout_until_s_by_idx.append(0)
        self.size += 1
